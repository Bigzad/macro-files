// FINAL CORRECTED: Supabase Edge Function - process-invite
// Fixed column names to match actual user_profiles table schema
...
serve(async (req)=>{
  ...
  try {
    ...
    if (code) {
      // INVITATION CODE PROCESSING - Check actual invitation_codes table first
      console.log('Processing invitation code:', code);
      // Check the correct table name 'invitation_codes' 
      const { data, error } = await supabaseClient.from('invitation_codes').select('*').eq('code', code).single();
      if (error || !data) {
      ...
      // Check if code is still valid (only if expires_at is set)
      if (data.expires_at) {
        const now = new Date();
      ...
      // Use correct column name 'usage_count' instead of 'current_uses'
      if (data.usage_count >= data.max_uses && data.max_uses > 0) {
        return new Response(JSON.stringify({
          error: 'This invitation code has no remaining uses'
        }), {
          status: 400,
          headers: {
            ...corsHeaders,
      ...
      invitationData = data;
      userRole = 'client'; // Default to client for invitation codes
      assignedCoach = data.coach_email; // Assign to the coach who created the code
    } else if (token) {
      // EMAIL TOKEN PROCESSING (existing logic)
      console.log('Processing email invitation token');
      const { data: invitation, error: inviteError } = await supabaseClient.from('invites').select('*').eq('token', token).eq('email', email).is('used_at', null).gt('expires_at', new Date().toISOString()).single();
      ...
      userRole = 'client'; // Email invitations assign CLIENT role by default
    }
    ...
    // FIXED: Create user profile with correct column names matching actual schema
    ...
    const { error: profileError } = await supabaseAdmin.from('user_profiles').insert({
      ...
      coach_user_id: invitationData?.coach_user_id || null,  // ADD: Set coach_user_id
    ...
    // 🚀 NEW: CREATE COACH ASSIGNMENT if user was assigned to a coach via invitation code
    if (code && invitationData?.coach_user_id) {
      ...
      if (assignmentError) {
        ...
        console.error('Coach assignment failed but user creation succeeded');
      } else {
        console.log('✅ Coach assignment created successfully');
      }
    } else {
      console.log('ℹ️ No coach assignment needed:', {
        hasCode: !!code,
        hasCoachUserId: !!invitationData?.coach_user_id
      });
    }

    // Mark invitation as used with correct table and column names
    if (token) {
      ...
      // Update invitation code usage in correct table with correct column name
      console.log('Updating invitation code usage:', {
        code: code,
        currentUsage: invitationData.usage_count,
        newUsage: invitationData.usage_count + 1
      });
      const { error: updateError } = await supabaseClient.from('invitation_codes').update({
